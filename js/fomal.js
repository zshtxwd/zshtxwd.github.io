...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 雨打窗前🌧️")[0];
}
...
$.ajax({
  ...
  data: {
    key: '7CYBZ-WU26G-DJCQT-QF2YS-TYT4H-7IBKP',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function createtime1() {
  var grt = new Date("06/20/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 雨打窗前🌧️") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('雨打窗前🌧️的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = '☔快进来躲雨~';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = '🌂伞就放一边吧~';
    //两秒后变回正常标题
...
if (m == 1 && dd == 27) {//站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝站长" + (y - 2001).toString() + "岁生日快乐！🌧️");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 6 && dd == 14) {//妍妍生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝妍妍宝贝" + (y - 2001).toString() + "岁生日快乐！🌸");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("06/20/2023 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
    ? `<img class='boardsign' src='https://imgbed.zshlmy.love/badge/F小屋-科研摸鱼中.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
    : `<img class='boardsign' src='https://imgbed.zshlmy.love/badge/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    ...
    --mobileday-bg: url(https://imgbed.zshlmy.love/picture/移动端白天.png);
    --mobilenight-bg: url(https://imgbed.zshlmy.love/picture/移动端黑夜.png);
  }`;
}
// 切换背景主函数
function changeBg(s) {
